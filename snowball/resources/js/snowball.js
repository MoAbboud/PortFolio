...
class SnowballCreator {
    ...
    buildTreeStructure() {
        ...
        for (let i = 1; i < this.pathArray.length; i++) {
            ...
            const levelNodes = currentOptions.map(option => ({
                text: option,
    ...
    createTreeNode(nodeData, levelIndex, index) {
        ...
        nodeDiv.className = `tree-node ${nodeData.type} animate-in`;
    ...
    setupLabelOptions(optionSetIndex) {
        ...
        optionBoxes.forEach((box, index) => {
            ...
            // Update option text
            const textElement = box.querySelector('.text-sm');
    ...
    completeSelection(optionBox) {
        const optionText = optionBox.querySelector('.text-sm').textContent;
