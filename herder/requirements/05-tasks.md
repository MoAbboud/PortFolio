...
**Stage 0 is built and verified except for the container checks, which need Docker.
Stage 1 - ingest - is the current work.**
...
**Built 2026-09-08. 27 tests pass with no database.** The three container checks are
marked `[~]` and need Docker: `docker compose up`, `alembic upgrade head`, and watching
`/health` actually go red against a real stopped database rather than a stubbed one.
