...
| `llm_calls` | Every model call, with its prompt version, tokens, latency and raw response |
...
`llm_calls`) and four are the measurement chain (`probes`, `checkpoints`, `probe_results`,
...
The full DDL is in the specification this folder was written from and is reproduced in the
first migration, which is hand-written rather than autogenerated so that the constraints
carrying the design are visible in it.
...
`embedding` is `vector(1536)` over `title + text` at the current revision, with an HNSW
index, and it is what the merge step searches. The dimension is fixed in the DDL, so
changing the embedding provider is a migration and not a configuration change. That is
recorded as an open question rather than hidden in a config file.
...
One row per call: purpose (`extract`, `adjudicate`, `probe_gen`, `judge`, `answer`), model,
prompt version, prompt tokens, completion tokens, latency, whether the response parsed on
the first attempt, and the raw response text. The raw response is the expensive part to keep
and the only one that matters when something breaks, because a parse failure with the
response thrown away is unfixable.

It also makes re-scoring free. Changing how a judge's output is interpreted, or how recall
is aggregated, is then a re-read of rows that have already been paid for rather than a fresh
run against a provider. A harness that costs money every time it is run is a harness that
stops being run.
...
9. Every LLM call that reaches a provider writes an `llm_calls` row, including the ones that
   fail. A failed call with no row is a cost nobody can account for.
...
sibling project's decision for the same reason, and the same caveat applies: `llm_calls` is
in the database because it is operational - it is how a live system explains its own cost -
not because it is part of the harness.
...
| `vendor_accounts` | Encrypted per-workspace provider keys. In v0.1 the key comes from the environment for the whole deployment, which is what a single-user self-hosted tool needs. The table arrives when someone other than the author has an account |
