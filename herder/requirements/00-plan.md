...
| 2 | Extract | One chunk goes to a model and comes back as candidate entries with lineage that validates against the chunk. Every call writes an `llm_calls` row with its prompt version, tokens, latency and raw response. Malformed JSON, invented lineage and a timeout are handled as three different things |
...
| 6 | Verify | Probe generation cached per revision, `api`-mode checkpoints, judging, the integrity score with its three weighted categories, and a suggestion for every zero |
...
| **This project requires a model API key, and that breaks the portfolio's keyless rule** | Deriving entries, adjudicating merges and grading probes are language-model work, and there is no heuristic substitute that would be measuring the same thing. The sibling project could replace its model with regular expressions and still be the same project. This one could not - the thing being measured *is* the model's ability to compact a conversation. The key is the user's own, from the environment, and the repository never ships one |
...
| `workspaces` kept from the start, `workspace_members` and `vendor_accounts` deferred | Retrofitting a tenancy boundary moves a foreign key on `projects` and touches every query. Adding a membership table later is additive and touches nothing |
| An `llm_calls` table, with the raw response kept | The specification asks for every call to be logged so the benchmark is reproducible; this is where that lives. Keeping the raw response makes re-scoring free, and a harness that costs money every run is a harness that stops being run |
| The API never calls a model. Only the worker does | Ingest has to be fast and cheap enough for a browser extension to hammer it, and a slow provider must not become a slow API |
...
| Brief versions are immutable; an adjust re-renders rather than edits | A checkpoint score has to refer to an exact text that still exists. It also makes the adjuster feel immediate, because re-rendering costs nothing and re-extracting costs money |
...
| --- | --- | --- |
| **What does one derive actually cost?** | Stage 2 | The most important open question in the project. A 60k-token conversation chunked at 6000 tokens is ten extraction calls plus adjudications plus embeddings. If that costs more than a few cents, "leave it running over your conversations" is not a thing anyone will do, and the threshold, the chunk size and the model choice all move. Measure it on the first real chunk and write the number down |
| Which model for extraction, and which for judging | Stage 2 | Start mid-tier and cheap, make it configuration. Whether the judge should be a different model from the extractor is a real question: the same model grading its own extraction is a conflict of interest that the hand-written ground truth only partly protects against |
| Which embedding provider | Stage 3 | The pgvector dimension is fixed at 1536 in the DDL, so this is a migration rather than a setting. Decide before stage 3 writes any embeddings |
| Is 0.86 the right similarity threshold | Stage 4 | Too low and distinct entries get adjudicated constantly, which costs money; too high and the memory grows linearly and the whole compaction claim fails. Only stage 4 can say |
| Is 6000 the right chunk size | Stage 4 | Bigger chunks mean fewer calls and more missed detail. Smaller chunks mean more calls and more duplicate candidates to merge. Both directions cost money |
...
| Are six probes enough to be a number | Stage 6 | Six probes gives a score with a granularity of about 0.08. That may be too coarse to detect the improvements stage 10 is looking for, and more probes cost money per checkpoint |
...
| Where does it host, and what does it cost | Stage 11 | Part of the deliverable, so not optional. A container plus a hosted PostgreSQL **with pgvector**, which is a smaller set of free tiers than plain PostgreSQL |
| What is a visitor to the hosted demo allowed to do | Stage 11 | An open paste box on a public link is an open invoice for provider tokens. Likely a read-only demo over a seeded project, with the paste box behind a rate limit or disabled |
...
| Cost per derive makes it unrunnable | The demo cannot be left running, and the harness stops being run | Measure the cost of a single real chunk at stage 2, before the loop is finished, and treat the number as a design input rather than a detail. Every model call is logged with tokens, so cost per project is a query rather than a guess |
