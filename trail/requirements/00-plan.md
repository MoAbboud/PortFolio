...
| Objects can be drawn as a smooth surface rather than as cubes | The user's request, after seeing that a canopy eleven cubes across cannot look round. Surface Nets over the same voxel grid, with roundness as a dial rather than a switch, and the cubes kept so the two can be compared. **Authoring does not change**: recipes still describe solids and the voxeliser still produces a grid. The honest cost is that a smoothed Trail is a low-poly diorama rather than a field of cubes, which is a different visual identity from the one the earlier documents describe |
