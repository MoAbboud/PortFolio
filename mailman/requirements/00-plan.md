...
A document intake pipeline that turns unstructured invoices into validated records, sends
the doubtful ones to a person, and can say - with a number it measured itself - how often it
gets each field right.

Finished, for the purpose of this project, when three things are true: the pipeline runs end
to end on a document nobody prepared for it, the review queue is usable by someone who is
not the author, and the README carries accuracy figures produced by the harness in this
repository, including the ones that did not improve.

The wider objective is what it demonstrates. Structured data moving between systems is
familiar ground; the new part is a model sitting inside that pipeline as one component among
several, with rules and a person around it. The evaluation harness is the part most portfolio
projects do not have, and it is what turns "I built this" into "line-item accuracy went from
x to y when I changed z, and here is the harness that told me".
...
flowchart LR
    ...
    subgraph C[Measurement]
        S7[7. Corpus<br/>and baseline] --> S8[8. Iteration]
    end
    subgraph D[Interface]
        S9[9. Review queue] --> S10[10. Deploy<br/>and README]
    end
    S3 --> S4
    S6 --> S7
    S8 --> S9
...
| --- | --- | --- |
| 0 | Scaffold | FastAPI, Docker Compose with PostgreSQL, Alembic migrations for all seven tables, `/health` green, git initialised with real messages from the first commit |
| 1 | Ingestion | `POST /documents` takes a PDF or image, stores the bytes in an S3-shaped path, inserts the row, returns the id, and pdfplumber has pulled the text |
...
| 7 | Corpus and baseline | Thirty to forty documents with labels beside them, a harness that reports per-field accuracy, and **a recorded baseline** |
| 8 | Iteration | At least three genuine attempts at improvement, each measured, each written into `NOTES.md`, including the ones that failed |
| 9 | Review queue | A person can clear a queue of ten in a browser and the corrections are logged |
| 10 | Deploy and README | Public link, and a README with the problem, the diagram, the numbers, the dead ends and the limitations |
...
because that list is a description of how this model fails on these documents rather than a
list of failures that seemed plausible in advance. The rules that come out of it are the
ones that catch something.
...
| Routing threshold is configuration | The trade between reviewer time and bad records is a number to be measured, and its justification written down |
...
| FastAPI, Docker Compose, Alembic from the first commit | A schema that was hand-built once cannot be rebuilt on another machine. The OpenAPI page is a free artifact |
...
| How are scans with no text layer handled - page image to a vision model, or OCR first? | After stage 3 | Affects cost and the shape of ingestion. Probably the image, with OCR as a fallback |
| How are line items matched between the extraction and the label? | Stage 7 | Order is not guaranteed. Probably matched on description then compared, but the matching itself can fail and that has to be visible rather than counted as a wrong amount |
| What exactly goes into composite confidence, and at what weights? | Stage 5 | The first version can be crude. The defensible version comes out of stage 7 |
| What is the threshold? | Stage 8 | Only a measurement can answer it. Pick something arbitrary in stage 5 and expect to change it |
| Does supplying similar labelled documents help at this corpus size? | Stage 8 | Thirty examples may be too few for it to do anything. Worth trying and worth reporting if it does not |
...
| Where does it deploy, and what does hosted PostgreSQL cost? | Stage 10 | Free database tiers have got worse. The public link is worth something, but not a monthly bill for a portfolio project |
...
| --- | --- | --- |
| Stages 7 and 8 get skipped because the demo already works | The project becomes indistinguishable from every other document-AI demo, and the one differentiating thing is the one missing | The stage order exists for this. No review UI work starts before the baseline is recorded |
...
| The review UI grows into an application | Time spent on a queue interface nobody was going to grade | It is a list, a document viewer and a form. Server-rendered, no build step |
...
| Stages 11 and 12 get started before stage 10 is finished | The interesting extensions half-built, the core unfinished | Both are optional and both come after a deployed, documented system |
