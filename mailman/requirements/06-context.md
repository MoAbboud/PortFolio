...
### 2026-09-04 - stage 8: the corpus at 37, and the baseline recorded at 75.1%

**What was asked.** "go for it" - stage 8. The pivot: everything before this was building,
everything after is measured.

## The corpus, 11 to 37

The eleven hand-written cases stay, each for one known-hard thing. **Twenty-four more are
generated by `_invoice`, which renders a document and derives its labels from the same values.**
Hand-typing twenty-four more answer keys was the alternative, and `02-many-lines` is the
standing evidence for what that costs - twelve computed line items under three typed totals
that disagreed with them by 182.00, sitting in the corpus looking clean.

Variety is deliberate rather than random: four label wordings, four table headers, five date
formats, five currencies, European separators, one to twenty-four line items. What is *not*
varied is correctness - the generated documents are all internally consistent, because a corpus
needs a majority of good documents for an accuracy figure to mean anything, and the awkward
ones are the eleven plus the refusal case.

**Two documents the pipeline cannot handle**, and this is the part worth defending:
`36-scan-no-text-layer` is a structurally valid PDF that yields an empty string - what a scan
is to pdfplumber - and `37-spreadsheet-columns-out-of-order` is a CSV. `Case` gained
`unsupported`, `raw` and `suffix`, and `build_image_only_pdf` makes the first. The harness
reports them separately and never as wrong. **An unsupported count in every run is a roadmap;
a document quietly kept out of the corpus is a forgotten TODO.**

## The harness

`python -m mailman.eval run --corpus ./corpus --label baseline`, writing to `evaluations/`,
never overwritten, in git. Measurement belongs where it can be read in a diff, and production
tables carrying `eval_runs` would put the harness inside the schema it is judging.

It drives the real extractor through the shared comparison in `corpus_check.py`. Four field
kinds - exact for `invoice_number` and `currency`, normalised for names, date for dates,
decimal for amounts - because scoring all four as string equality reports a full stop as an
extraction failure and `270.0` against `270.00` as a wrong number. Line items are matched as a
**set** on the normalised description, with precision and recall, so a shuffled document is not
wrong about its lines and a failed match shows up as a recall miss rather than four wrong
fields on a row that was never there.

## The number I nearly recorded was 99.7%

The first version skipped refused documents, and the corpus reported **99.7% while producing
nothing at all for twelve of thirty-four documents.** That is precisely the dishonesty this
harness exists to prevent, and it has a sharper form: **a system that refuses everything it
finds hard would score 100%.** Scoring a refusal as silence rewards refusing.

A refusal now counts every field on the document as wrong, which is what it is. The headline
moved from 99.7% to 75.1% and the second number is the true one.

## THE BASELINE

    OVERALL          75.1%   (329/438)        evaluations/20260904T194049-baseline.json
...
**Almost all of it is one problem twelve times.** Twelve documents are refused over
`invoice_number` because `_INVOICE_NUMBER` knows only `invoice`, `inv` and `credit note`, and
those twelve say `Our reference` or `Document ID`. A refusal takes every field with it, which
is why nine unrelated fields sit in the low sixties - **the per-field table is not showing nine
problems.** The thirteenth wrong field is `Fairview Practice` read as `Fairview`.

**Not fixed, deliberately.** A baseline taken after the first improvement is not one. The fix
is one line and it belongs to stage 9, where it gets measured.

## Making the suite honest about it

Growing the corpus turned 24 tests red, and a permanently red suite is worse than none. Two
mechanisms, both already in the project:

- **`KNOWN_GAPS` is now derived** - `{(case, "buyer_name") for every case that labels one}` -
  rather than a hand-listed set of eleven. The heuristic categorically does not attempt a
  buyer, so the exemption is categorical and a hand-maintained list of thirty-four names would
  have been wrong the moment the corpus grew again.
- **The twelve refusals are `xfail(strict=True)`**, not skips. When stage 9 teaches the
  extractor those labels, every fixed document turns the suite red with "unexpectedly passing"
  and the entry has to come out. **An xfail that can stay after the bug is fixed is a skip
  wearing a costume.**

Unsupported documents are excluded from the regression tests entirely rather than xfailed:
asserting the pipeline extracts a scan would be asserting a feature nobody has built.

**And the `expected`-key guard fired again**, on `unsupported_reason` - a key I had put in the
labels that duplicated `case.unsupported`. Removed. That guard has now caught three separate
attempts to put something in `expected` that nothing evaluates.

**Verification.** 285 passed, 24 xfailed, 1 skipped. 21 new harness tests, and the ones worth
reading are the two that pin the honesty: a refusal counts as wrong, and an unsupported
document does not.
...
| File | Change |
| --- | --- |
| `mailman/eval.py` | New. The harness and its CLI |
| `mailman/corpus.py` | `_invoice`, 24 generated cases, 2 unsupported; `Case` gains `unsupported`, `raw`, `suffix` |
| `mailman/pdfwriter.py` | `build_image_only_pdf` |
| `mailman/corpus_check.py` | `KNOWN_GAPS` derived from the corpus |
| `tests/test_eval.py` | New, 21 tests |
| `tests/test_corpus.py` | `BASELINE_GAPS` as strict xfails; unsupported excluded |
| `evaluations/` | New, in git, append-only |
| `NOTES.md` | **The baseline** |

**Next.** Stage 9: three genuine attempts at improvement, each measured against 75.1%, each
written into `NOTES.md` **including the ones that fail**. The first is obvious and already
diagnosed - the invoice-number label list - and the discipline is one change at a time, which
this project has broken once and paid for.

The styling gate also lifts now: the baseline exists, so stage 7's queue is allowed a pass.
It is still worth doing the measurement first.

**Uncommitted.** Everything remains in the working tree by request.

### 2026-09-04 - stages 9 and 10: 75.1% to 98.3%, and the README

**What was asked.** "okay go forth and complete the app."

## Stage 9, three attempts, each measured
...
Every run is in `evaluations/`, append-only.

**Attempt 1 was not hard because of the labels.** Twelve documents were refused because
`_INVOICE_NUMBER` knew only `invoice`, `inv` and `credit note` while they said `Our reference`
or `Document ID`. The difficulty was `10-not-an-invoice`, a delivery note reading `Delivery
Reference: DN-4471` - a pattern matching `reference` anywhere would have mined an invoice
number out of the one document the corpus exists to refuse, trading twelve wrong refusals for
a false acceptance. Anchoring the new labels to the start of a line threads it, because an
invoice number's label starts its line and `Delivery Reference` starts with `Delivery`.

**Attempt 2 broke the tax field while fixing the subtotal one.** `Total excl. tax` is a real
way to write a subtotal and also contains the word `tax`, so the tax lookup reached the
subtotal line first and five documents reported 576.00 where the answer was 115.20. A plausible
number in the wrong field, produced by widening a vocabulary - the shape of every silent bug
this extractor has had.

**The harness caught it through `arithmetic breaks` going 1 to 6**, not through the tax rate.
Third time that check has found something no label comparison did, and it still needs no answer
key. Fixed with an `exclude` list so the tax lookup skips subtotal lines.

**Attempt 3 disproved a claim that had stood since stage 2.** `buyer_name` was left null on the
grounds that "no rule finds a buyer reliably" - never measured. A rule reading the text after a
buyer label gets **33 of 34 against the trained model's 30**, because the model truncates
multi-word names.

But every corpus document says `Bill To:`, and this project has been burned by exactly that
convention trap before, so the rule was probed against wordings the corpus does not contain:

    Bill To: .. Buyer     rule correct        model correct
    Deliver invoice to    rule correct        model correct
    Purchaser             rule NONE           model correct
    Attention             rule NONE           model correct

**Neither wins, and the split is the project's whole thesis in one table.** The rule is exact
when it matches and blind when it does not; the model is robust to unseen wording and sloppy
about where a name ends. The hybrid overlays the model only where the rule found nothing, so it
gets both - and that is now measured rather than asserted.

The 250MB stays justified on a narrower and truer claim: not "the model finds buyers" but "the
model finds buyers on documents whose wording the rules have never seen".

**All 23.2 points came from vocabulary breadth**, which is the same finding the trained model
produced over five runs from the opposite direction. Rules and a tagger failed for the same
reason and were fixed by the same insight.

## The suite went red on purpose, and that was the design working

Stage 8 marked twelve documents `xfail(strict=True)`. Fixing them turned the suite red with
"unexpectedly passing" until each entry came out - twenty-three failures that were all good
news. Three gaps remain.

`KNOWN_GAPS` is now **empty**, and `test_known_gaps_are_still_gaps` is what forced it: it held
`buyer_name` on every document for eight stages on a claim nobody had tested, and the test made
the exemption impossible to leave behind once the claim was disproved.

Three other tests referenced documents that had changed - a refusal case that stopped refusing,
a wrong-field case that stopped being wrong, and a correction test whose "change" became a
no-op once the heuristic found the buyer itself. All repointed with a note saying why.

## Stage 10, the README

Rewritten around the numbers. It leads with 98.3% against a recorded 75.1% baseline, and it
carries the things that did not work, because those are the part that is hard to fake:

- vocabulary recorded as a dead end that was actually a half-applied change;
- the ablations cancelled because measured variance said they could not support a conclusion;
- 84.1% being the best of three draws, with 79.0% and its range as the honest figure;
- the harness that nearly reported 99.7% by skipping what it refused;
- two rules deleted from the original design after contact with real documents.

Every figure in it was verified against a live run rather than transcribed.

**What is left: hosting.** It is the fourth item in the plan's definition of done and the only
one outstanding. It is also the author's - it needs his accounts, and the three questions in
"Still open" are his to answer: what a container plus a hosted Postgres costs now, whether the
database ships seeded (a link that opens on an empty queue demonstrates nothing), and what a
visitor is allowed to do, because an open upload box on a public link is an open invoice for
somebody's tokens.

**Verification.** 303 passed, 6 xfailed, 1 skipped. 98.3% on the corpus.

**Every change made this pass.**

| File | Change |
| --- | --- |
| `mailman/heuristic.py` | `_REFERENCE_NUMBER`; `_DUE_LABELS`, `_SUBTOTAL_LABELS`, `_TAX_LABELS`, `_ISSUE_LABELS`, `_BUYER_LABELS` in one place; `_labelled_amount` gains `exclude`; `_buyer_name` |
| `mailman/corpus_check.py` | `KNOWN_GAPS` emptied, with the reason |
| `tests/` | `BASELINE_GAPS` twelve to three; four tests repointed |
| `README.md` | Rewritten around the measurements |
| `NOTES.md`, `requirements/05-tasks.md` | Stage 9 recorded |
| `evaluations/` | Six runs |

**Next.** Stage 10's hosting, then the two optional stages - an MCP wrapper over the API that
